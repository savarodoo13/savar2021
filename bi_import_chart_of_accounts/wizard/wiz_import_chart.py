...
class ImportChartAccount(models.TransientModel):
	...
	def copy_accounts(self):
		...
		if 'create_asset' in self.env['account.account']._fields:
			self.env.cr.execute(""" 
				INSERT INTO ACCOUNT_ACCOUNT (NAME,CODE,CURRENCY_ID,DEPRECATED,USER_TYPE_ID,INTERNAL_TYPE,INTERNAL_GROUP,RECONCILE,NOTE,COMPANY_ID,
				...
				SELECT NAME,CODE,CURRENCY_ID,DEPRECATED,USER_TYPE_ID,INTERNAL_TYPE,INTERNAL_GROUP,RECONCILE,NOTE, %d AS COMPANY_ID,
				GROUP_ID,M_CLOSE,ACCOUNT_TYPE_IT_ID,ACCOUNT_TYPE_CASH_ID,PATRIMONY_ID,TYPE_ADQUISITION,CHECK_MOORAGE,
	...
	def create_chart_accounts(self,values):
		...
		account_search = account_obj.search([
			('code', '=', code_no),('company_id','=',self.env.company.id)
			])

		if account_search:
			#raise UserError('Ya Existe la cuenta %s' % (code_no))
