...
class AccountMove(models.Model):
	...
	ple_state = fields.Selection([('1','Fecha del Comprobante Corresponde al Periodo'),
								...
								('9','Corresponde a un Periodo Anterior y si ha sido Anotado en dicho Periodo')],string='Estado para el PLE', default='1', copy=False)
	date_corre_ple = fields.Date(string='Fecha Correccion PLE', copy=False)
	...
	type_op_det = fields.Char(string=u'Tipo de Operación',size=2, copy=False)
	...
	def _check_duplicate_supplier_reference(self):
		...
		if duplicated_moves:
			raise ValidationError(_('Duplicated vendor reference detected. You probably encoded twice the same vendor bill/credit note:\n%s') % "\n".join(
				duplicated_moves.mapped(lambda m: "%(partner)s - %(ref)s - %(type_document)s - Registro SUNAT(Diario):%(register_sunat)s" % {'ref': m.ref, 'partner': m.partner_id.display_name, 'type_document': m.type_document_id.name, 'register_sunat': m.journal_id.register_sunat})
			))
