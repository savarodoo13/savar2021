...
class ImportJournalEntryIt(models.TransientModel):
	...
	def make_move_line(self,values,move_id):
		...
		if values.get("analytic_account_id"):
			s = str(values.get("analytic_account_id"))
			analytic = s.rstrip('0').rstrip('.') if '.' in s else s
			analytic_account_id = self.find_analytic_account(analytic)
		...
		s = str(values.get("account_id"))
		account = s.rstrip('0').rstrip('.') if '.' in s else s
