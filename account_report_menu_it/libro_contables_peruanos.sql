...
CREATE OR REPLACE VIEW public.vst_compras_1_bm AS 
 SELECT a1.move_id,
    ...
    sum(
        CASE
            WHEN a1.col_reg::text = 'BASE1'::text THEN a3.tax_amount_me
            ELSE 0::numeric
        END) AS base1me,
    sum(
        CASE
            WHEN a1.col_reg::text = 'BASE2'::text THEN a3.tax_amount_me
            ELSE 0::numeric
        END) AS base2me,
    sum(
        CASE
            WHEN a1.col_reg::text = 'BASE3'::text THEN a3.tax_amount_me
            ELSE 0::numeric
        END) AS base3me,
    sum(
        CASE
            WHEN a1.col_reg::text = 'CNG'::text THEN a3.tax_amount_me
            ELSE 0::numeric
        END) AS cngme,
    sum(
        CASE
            WHEN a1.col_reg::text = 'IGV1'::text THEN a3.tax_amount_me
            ELSE 0::numeric
        END) AS igv1me,
    sum(
        CASE
            WHEN a1.col_reg::text = 'IGV2'::text THEN a3.tax_amount_me
            ELSE 0::numeric
        END) AS igv2me,
    sum(
        CASE
            WHEN a1.col_reg::text = 'IGV3'::text THEN a3.tax_amount_me
            ELSE 0::numeric
        END) AS igv3me,
    sum(
        CASE
            WHEN a1.col_reg::text = 'ICBPER-C'::text THEN a3.tax_amount_me
            ELSE 0::numeric
        END) AS icbperme,
    sum(
        CASE
            WHEN a1.col_reg::text = 'ISC-C'::text THEN a3.tax_amount_me
            ELSE 0::numeric
        END) AS iscme,
    sum(
        CASE
            WHEN a1.col_reg::text = 'OTROS-C'::text THEN a3.tax_amount_me
            ELSE 0::numeric
        END) AS otrosme
   FROM vst_diariog a1
...
CREATE OR REPLACE VIEW public.vst_ventas_1_bm AS 
 SELECT a1.move_id,
    ...
    sum(
        CASE
            WHEN a1.col_reg::text = 'VENTA-G'::text THEN - a3.tax_amount_me
            ELSE 0::numeric
        END) AS ventagme,
    sum(
        CASE
            WHEN a1.col_reg::text = 'EXP'::text THEN - a3.tax_amount_me
            ELSE 0::numeric
        END) AS expme,
    sum(
        CASE
            WHEN a1.col_reg::text = 'EXO'::text THEN - a3.tax_amount_me
            ELSE 0::numeric
        END) AS exome,
    sum(
        CASE
            WHEN a1.col_reg::text = 'INAF'::text THEN - a3.tax_amount_me
            ELSE 0::numeric
        END) AS inafme,
    sum(
        CASE
            WHEN a1.col_reg::text = 'ISC-V'::text THEN - a3.tax_amount_me
            ELSE 0::numeric
        END) AS iscme,
    sum(
        CASE
            WHEN a1.col_reg::text = 'OTROS-V'::text THEN - a3.tax_amount_me
            ELSE 0::numeric
        END) AS otrosme,
    sum(
        CASE
            WHEN a1.col_reg::text = 'IGV-V'::text THEN - a3.tax_amount_me
            ELSE 0::numeric
        END) AS igvme,
    sum(
        CASE
            WHEN a1.col_reg::text = 'ICBPER-V'::text THEN - a3.tax_amount_me
            ELSE 0::numeric
        END) AS icbperme
   FROM vst_diariog a1
     LEFT JOIN account_journal a2 ON a2.id = a1.journal_id
