...
class WebsiteSaleInherit(WebsiteSale):
    ...
    def _checkout_form_save(self, mode, checkout, all_values):
        ...
        if mode[0] == 'new':
            partner_exis = Partner.sudo().search([('vat','=',checkout['vat']),('l10n_latam_identification_type_id','=',checkout['l10n_latam_identification_type_id'])])
            if len(partner_exis)>0:
                partner_exis[0].sudo().with_context(tracking_disable=True).write(checkout)
    ...
    def _get_mandatory_billing_fields(self):
        return ["name", "email", "street", "country_id","l10n_latam_identification_type_id","vat"]

    def _get_mandatory_shipping_fields(self):
        return ["name", "street",  "country_id","l10n_latam_identification_type_id","vat"]
    ...
    def address(self, **kw):
        ...
        l10n_latam_identification_type_id = request.env['l10n_latam.identification.type'].search([])
