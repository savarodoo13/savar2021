...
class AccountMove(models.Model):
	...
	def generate_excel_rep_it(self):
		...
		for line in self.line_ids:
			...
			worksheet.write(x,1,line.partner_id.vat + ' ' + line.partner_id.name if line.partner_id else '',formats['especial1'])
	...
	def generate_pdf_rep_it(self):
		...
		def pdf_header(self,c,wReal,hReal,size_widths):
			if not self.company_id.partner_id.street or not self.company_id.partner_id.state_id or not self.company_id.partner_id.vat:
				raise UserError('El partner asociado a la compañia no tiene configurado Calle, Region o RUC')
			...
			c.drawString(30,hReal-20,particionar_text( self.company_id.partner_id.street,100))
			c.drawString(30,hReal-30, self.company_id.partner_id.state_id.name)
			c.drawString(30,hReal-40, self.company_id.partner_id.vat)
		...
		for line in self.line_ids:
			...
			c.drawString( first_pos+2 ,pos_inicial,particionar_text( line.partner_id.vat + ' ' + line.partner_id.name if line.partner_id else '',150) )
