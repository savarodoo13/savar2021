		...
		for row_no in range(sheet.nrows):
			if row_no <= 0:
				...
				if self.partner_option == 'create':
					...
					if line[17]:
						saleperson_search = self.env['res.users'].search([('name','=',line[17])],limit=1)
						if not saleperson_search:
							raise Warning("Usuario no disponible en el sistema")
						else:
							saleperson = saleperson_search.id
					if line[19]:
						cust_payment_term_search = self.env['account.payment.term'].search([('name','=',line[19])],limit=1)
						if not cust_payment_term_search:
							raise Warning(u"Término de pago no disponible en el sistema")
						else:
							cust_pmt_term = cust_payment_term_search.id
					if line[20]:
						vendor_payment_term_search = self.env['account.payment.term'].search([('name','=',line[20])],limit=1)
