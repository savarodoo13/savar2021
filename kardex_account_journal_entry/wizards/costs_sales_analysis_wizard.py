...
class CostsSalesAnalysisWizard(models.TransientModel):
	...
	def make_invoice(self):
		...
		for elem in dic_debit:
			...
			lineas.append(vals)
		self.env.cr.execute("""select almacen,input_account_id,ROUND(SUM(coalesce(valor,0)),2) as credit from costs_sales_analysis_book
	...
	def _get_sql(self,date_ini,date_end,company_id):
		...
		sql = """SELECT
				...
				ELSE (SELECT account_id FROM vst_property_stock_account_output WHERE company_id = {company} LIMIT 1)
				...
				WHERE ST.usage = 'internal' AND ST2.usage = 'customer'
				UNION ALL
				SELECT 
				GKV.almacen,
				GKV.name_template,
				...
				LEFT JOIN stock_location ST ON ST.id = GKV.ubicacion_origen
				LEFT JOIN stock_location ST2 ON ST2.id = GKV.ubicacion_destino
				WHERE ST.usage = 'customer' AND ST2.usage = 'internal')T
				GROUP BY almacen,
				product_id)T2
