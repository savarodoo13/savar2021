...
class AccountBalancePeriodRep(models.TransientModel):
	...
	def get_excel(self):
		...
		worksheet = workbook.add_worksheet("Saldo por Fecha Cont")
		worksheet.set_tab_color('blue')

		HEADERS = ['PERIODO','FEC CON','LIBRO','VOUCHER','TDP','RUC','PARTNER','TD','NRO COMP','FEC DOC','FEC VEN','CUENTA','DEBE','HABER','SALDO MN','SALDO ME']
		...
		for line in self.env['account.balance.period.book'].search([]):
			...
			worksheet.write(x,14,line.saldo_mn if line.saldo_mn else 0,formats['numberdos'])
			worksheet.write(x,15,line.saldo_me if line.saldo_me else 0,formats['numberdos'])
			x += 1
		...
		worksheet.write(x,14,saldo_mn,formats['numbertotal'])
		worksheet.write(x,15,saldo_me,formats['numbertotal'])

		widths = [7,10,10,10,4,11,40,4,10,10,10,10,12,12,12,12]
		worksheet = ReportBase.resize_cells(worksheet,widths)
		workbook.close()
		f = open(direccion +'Saldo_por_Fecha_Cont.xlsx', 'rb')
		return self.env['popup.it'].get_file('Saldo_por_Fecha_Cont.xlsx',base64.encodestring(b''.join(f.readlines())))
