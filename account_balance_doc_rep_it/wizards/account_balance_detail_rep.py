...
class AccountBalanceDetailRep(models.TransientModel):
	...
	def get_excel(self):
		...
		HEADERS = ['PERIODO','FECHA','LIBRO','VOUCHER','TDP','RUC','PARTNER','TD','NRO COMP','FEC DOC','FEC VEN','CUENTA','DEBE','HABER','BALANCE','IMPORTE ME','SALDO MN','SALDO ME']
		...
		for line in self.env['account.balance.detail.book'].search([]):
			...
			worksheet.write(x,14,line.balance if line.balance else 0,formats['numberdos'])
			worksheet.write(x,15,line.importe_me if line.importe_me else 0,formats['numberdos'])
			worksheet.write(x,16,line.saldo if line.saldo else 0,formats['numberdos'])
		...
		worksheet.write(x,14,balance,formats['numbertotal'])
		worksheet.write(x,15,importe_me,formats['numbertotal'])

		widths = [7,10,10,10,4,11,40,4,10,10,10,10,12,12,12,12,12,12]
		worksheet = ReportBase.resize_cells(worksheet,widths)
		workbook.close()
		f = open(direccion +'Detalle_Comprobantes.xlsx', 'rb')
		return self.env['popup.it'].get_file('Detalle_Comprobantes.xlsx',base64.encodestring(b''.join(f.readlines())))
