...
class make_kardex_valorado_formato_sunat(models.TransientModel):
	...
	def reportpdf_sunat(self):
		...
		direccion = self.env['main.parameter'].search([('company_id','=',self.company_id.id)])[0].dir_create_file
	...
	def reporteador(self,obj_move):
		...
		for line in self.env.cr.fetchall():
			if line[21] != almacen_index or line[9] != product_index:
				...
				c.setFont("Calibri", 8)		
				c.drawString( 240,pos_inicial-20, str(date_ini) + ' - ' + str(date_fin))
				c.drawString( 240,pos_inicial-32, self.env['res.company'].search([])[0].partner_id.nro_documento or '' )
				c.drawString( 240,pos_inicial-44, self.env['res.company'].search([])[0].partner_id.name or '')
				c.drawString( 240,pos_inicial-56, line[21] or '')
				c.drawString( 240,pos_inicial-68, line[22] or '')
				producto_obj = self.env['product.product'].browse(line[24])
				c.drawString( 240,pos_inicial-80, producto_obj.categ_id.einvoice_05.code or '')
				c.drawString( 240,pos_inicial-92, line[9] or '')
				c.drawString( 240,pos_inicial-104,producto_obj.uom_id.einvoice_06.code or '')
				c.drawString( 240,pos_inicial-116, 'Costo Promedio')
			...
			c.setFont("Calibri", 7)	
			c.drawString( 10 ,pos_inicial, self.particionar_text( line[1] if line[1] else '',36) )
