...
class AccountMove(models.Model):
	...
	sunat_ticket_number = fields.Char(string='Numero de Ticket SUNAT')
	...
	def send_delete(self):
		...
		fact_line = next(filter(lambda s:s.serie_id == self.serie_id and s.billing_type == parameters.billing_type,parameters.serial_nubefact_lines))
	...
	def verify_invoice_data(self,parameters):
		...
		if not next(filter(lambda s:s.serie_id == self.serie_id and s.billing_type == parameters.billing_type,parameters.serial_nubefact_lines),None):
		...
		if advance_counter > 0 and self.op_type_sunat_id.pse_code in parameters.catalog_51_advance_ids.mapped('pse_code'):
	...
	def send_ebill(self):
		...
		for e_line in ebill_lines:
			...
			if self.billing_type == '1':
				...
				if e_line['codigo_tipo_afectacion_igv'] in ['11','12','13','14','15','16','21','31','32','33','34','35','36','37']:
		...
		if self.billing_type == '0':
			...
			ebill_json = {
				...
				"detraccion_tipo": int(self.detraction_type_id.pse_code) if self.op_type_sunat_id.code in DetractionCodes else "",
				...
				"observaciones": "Pedido: {0}\n{1}".format(self.invoice_origin, invoice_footer) if parameters.invoice_origin_check and self.invoice_origin else (invoice_footer),
				...
				"medio_de_pago": "",
				"placa_vehiculo": "",
				"orden_compra_servicio": (self.doc_origin_customer if self.doc_origin_customer else "") if parameters.doc_origin_customer_check else "",
		...
		if self.billing_type == '1':
			...
			ebill_json = {
				...
				"medio_de_pago": "",
				#"placa_vehiculo": "",
				"numero_orden_de_compra": (self.doc_origin_customer if self.doc_origin_customer else "") if parameters.doc_origin_customer_check else ""
			}
			if self.type_document_id.code in ['07','08']:
				ebill_json.update({'motivo_o_sustento_de_nota': self.credit_note_type_id.name if self.type_document_id.code in ['07'] else self.debit_note_type_id.name})
				tipo_nota = '01'
				for fac_relacionada in self.doc_invoice_relac:
					fact = self.env['account.move'].search([('ref','=',fac_relacionada.nro_comprobante),('type','=','out_invoice'),('type_document_id','=',fac_relacionada.type_document_id.id)])
					afecta_ext = ""
	...
	def check_format_numberg(self,numberg):
		...
		if not match:
			raise UserError(u'El formato del número de guía "'+numberg+'" es incorrecto, el formato debe ser similar a "0001-00001"')
