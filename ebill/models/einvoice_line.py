# -*- coding: utf-8 -*-

from odoo import models, fields, api

class EinvoiceLine(models.Model):
	_name = 'einvoice.line'

	move_line_id = fields.Many2one('account.move.line',ondelete='cascade')
	code = fields.Char(string='Codigo')
	uom = fields.Char(string='Unidad de Medida')
	unit_value = fields.Float(string='Valor Unitario',digits=(12,2))
	unit_price = fields.Float(string='Precio Unitario',digits=(12,2))
	discount_value = fields.Float(string='Valor Descuento',digits=(12,2))
	percentage_discount = fields.Float(string='Porcentaje Descuento',digits=(12,2))
	sunat_product_code = fields.Char(string='Codigo Producto SUNAT')
	igv_type = fields.Char(string='Tipo de IGV',size=2)
	subtotal = fields.Float(string='Subtotal',digits=(12,2))
	igv = fields.Float(string='IGV',digits=(12,2))
	icbper = fields.Float(string='ICBPER',digits=(12,2))
	total = fields.Float(string='Total',digits=(12,2))
	advance_regularization = fields.Boolean(string='Anticipo Regularizacion',default=False)
	advance_document_serie = fields.Char(string='Anticipo Serie Documento')
	advance_document_number = fields.Char(string='Anticipo Numero Documento')