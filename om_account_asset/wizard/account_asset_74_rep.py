...
class AccountAsset74Rep(models.TransientModel):
	...
	def get_report(self):
		self.env.cr.execute("""
				CREATE OR REPLACE view account_asset_74_book as ("""+self._get_sql_74(self.period.date_end,self.company_id.id)+""")""")
	...
	def _get_sql_74(self,date_period_end,company_id):
		sql = """select row_number() OVER () AS id,
				...
				where company_id = %s and contract_date <= '%s' and contract_date is not null and state <> 'draft'
				""" % (str(company_id),date_period_end.strftime('%Y/%m/%d'))
