...
class AccountAssetRep(models.TransientModel):
	...
	def get_excel(self):
		...
		HEADERS = ['CODIGO','ACTIVO','MES','PERIODO','CATEGORIA','CTA ANALITICA','CTA ACTIVO','CTA GASTO','CTA DEPRECIACION','VALOR DE DEPRECIACION']
		...
		for line in self.env['account.asset.book'].search([]):
			...
			worksheet.write(x,6,line.cta_activo if line.cta_activo else '',formats['especial1'])
			worksheet.write(x,7,line.cta_gasto if line.cta_gasto else '',formats['especial1'])
			worksheet.write(x,8,line.cta_depreciacion if line.cta_depreciacion else '',formats['especial1'])
			worksheet.write(x,9,line.valor_dep if line.valor_dep else '',formats['numberdos'])
			x += 1

		widths = [7,40,5,9,40,16,12,12,19,24]
